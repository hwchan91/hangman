...
Zzz}
  ...
  class NewGame
    ...
    def initialize(no)
      ...
      @answer_arr = answer.split("") if no == "1"
      @answer_arr = [] if no == "2"
      @guessed_char = []
    end
  end
  ...
  def run
    puts "-------New Game------"
    puts "Guess a word with #{@curr_game.answer_arr.length} characters." if @vs == "1"
    if @vs == "2"
...
puts %s{                |
  ...
  def guessing_turn
    ...
    until ("A".."Z").include? guess and !@curr_game.guessed_char.include? guess
      ...
      if guess.length != 1 or (!("A".."Z").include? guess)
...
#    @word_length = @curr_game.answer_arr.length
    begin
    ...
    if !(5..12).include? response
      ...
      @poss_words = poss_words.map {|word| word.upcase}
    end
  end
  ...
  def computer_check_guess(guess)
    ...
    confirm1 = confirm_func("Does your word contain the letter '#{guess}'? (Y/N)")
    if confirm1 == "Y"
      puts "Confirm: Are you sure your word contain the letter '#{guess}'? (Y/N)"
      double_confirm(true, confirm1, guess)
    else
      puts "Confirm: Are you sure your word does not contain the letter '#{guess}'? (Y/N)"
      double_confirm(false, confirm1, guess)
    end
  end

  def double_confirm(a, confirm1, guess)
    confirm2 = confirm_func("Invalid response.")
    if confirm2 == "Y"
      obtain_index if confirm1 == "Y"
      @contain_word = a
    else
      computer_check_guess(guess)
    end
  ...
  def obtain_index
    ...
    confirm = confirm_func("Please try again.")

    if confirm == "Y"
      update_word_display(@curr_game.answer_arr, response)
      @indexes = response
    else
      puts "haha"
      obtain_index
...
#      indexes = []
#      @curr_game.answer_arr.each_with_index do |char, index|
#        indexes << index if char == @guess
#      end

      @indexes.each do |i|
        @poss_words = @poss_words.select {|word| word[i-1] == @guess }
      end
      ((1..@word_length).to_a - @indexes.map{|i| i - 1 }).each do |i|
        @poss_words = @poss_words.select {|word| word[i] != @guess }
      end
    else
      @poss_words = @poss_words.select {|word| !word.include? @guess}
    end

    if @poss_words.length == 1 and @curr_game.answer_arr.any? {|letter| letter == ""}
      puts "I know! Is the answer is #{@poss_words[0]}? (Y/N)"
      confirm = confirm_func("Invalid response. Is the word '#{@poss_words[0]}'? (Y/N)")
      if confirm == "Y"
